...
pub fn hom_nand(
  ...
  *tlwe_nand.b_mut() = tlwe_nand.b().wrapping_add(utils::f64_to_torus(0.125));
  ...
  {
    bootstrap(&tlwe_nand, cloud_key, plan)
  }
...
pub fn hom_or(
  ...
  *tlwe_or.b_mut() = tlwe_or.b().wrapping_add(utils::f64_to_torus(0.125));
  #[cfg(feature = "bootstrapping")]
  {
    bootstrap(&tlwe_or, cloud_key, plan)
  }
  #[cfg(not(feature = "bootstrapping"))]
  {
    return tlwe_or;
...
pub fn hom_and(
  ...
  *tlwe_and.b_mut() = tlwe_and.b().wrapping_add(utils::f64_to_torus(-0.125));
  #[cfg(feature = "bootstrapping")]
  {
    bootstrap(&tlwe_and, cloud_key, plan)
  }
  #[cfg(not(feature = "bootstrapping"))]
  {
    return tlwe_and;
...
pub fn hom_xor(
  ...
  *tlwe_xor.b_mut() = tlwe_xor.b().wrapping_add(utils::f64_to_torus(0.25));
  #[cfg(feature = "bootstrapping")]
  {
    bootstrap(&tlwe_xor, cloud_key, plan)
  }
  #[cfg(not(feature = "bootstrapping"))]
  {
    return tlwe_xor;
...
pub fn hom_xnor(
  ...
  *tlwe_xnor.b_mut() = tlwe_xnor.b().wrapping_add(utils::f64_to_torus(-0.25));
  #[cfg(feature = "bootstrapping")]
  {
    bootstrap(&tlwe_xnor, cloud_key, plan)
  }
  #[cfg(not(feature = "bootstrapping"))]
  {
    return tlwe_xnor;
...
pub fn hom_nor(
  ...
  *tlwe_nor.b_mut() = tlwe_nor.b().wrapping_add(utils::f64_to_torus(-0.125));
  #[cfg(feature = "bootstrapping")]
  {
    bootstrap(&tlwe_nor, cloud_key, plan)
  }
  #[cfg(not(feature = "bootstrapping"))]
  {
    return tlwe_nor;
...
pub fn hom_and_ny(
  ...
  *tlwe_and_ny.b_mut() = tlwe_and_ny.b().wrapping_add(utils::f64_to_torus(-0.125));
  #[cfg(feature = "bootstrapping")]
  {
    bootstrap(&tlwe_and_ny, cloud_key, plan)
  }
  #[cfg(not(feature = "bootstrapping"))]
  {
    return tlwe_and_ny;
...
pub fn hom_and_yn(
  ...
  *tlwe_and_yn.b_mut() = tlwe_and_yn.b().wrapping_add(utils::f64_to_torus(-0.125));
  #[cfg(feature = "bootstrapping")]
  {
    bootstrap(&tlwe_and_yn, cloud_key, plan)
  }
  #[cfg(not(feature = "bootstrapping"))]
  {
    return tlwe_and_yn;
...
pub fn hom_or_ny(
  ...
  *tlwe_or_ny.b_mut() = tlwe_or_ny.b().wrapping_add(utils::f64_to_torus(0.125));
  #[cfg(feature = "bootstrapping")]
  {
    bootstrap(&tlwe_or_ny, cloud_key, plan)
  }
  #[cfg(not(feature = "bootstrapping"))]
  {
    return tlwe_or_ny;
...
fn bootstrap(ctxt: &Ciphertext, cloud_key: &CloudKey, plan: &mut FFTPlan) -> Ciphertext {
  let trlwe = blind_rotate(ctxt, cloud_key, plan);
...
mod tests {
  ...
  fn test_hom_nand() {
    test_gate(
      |a, b| !(a & b),
      |a: &Ciphertext, b: &Ciphertext, k: &CloudKey, p: &mut FFTPlan| gates::hom_nand(a, b, k, p),
    );
  }
  ...
  fn test_hom_or() {
    test_gate(
      |a, b| a | b,
      |a: &Ciphertext, b: &Ciphertext, k: &CloudKey, p: &mut FFTPlan| gates::hom_or(a, b, k, p),
    );
  }
  ...
  fn test_hom_xnor() {
    test_gate(
      |a, b| false ^ (b ^ a),
      |a: &Ciphertext, b: &Ciphertext, k: &CloudKey, p: &mut FFTPlan| gates::hom_xnor(a, b, k, p),
    );
  }
  ...
  fn test_hom_xor() {
    test_gate(
      |a, b| a ^ b,
      |a: &Ciphertext, b: &Ciphertext, k: &CloudKey, p: &mut FFTPlan| gates::hom_xor(a, b, k, p),
    );
  }

  #[test]
  fn test_hom_not() {
    test_gate(|a, _| !a, |a: &Ciphertext, _, _, _| gates::hom_not(a));
  }

  #[test]
  fn test_hom_copy() {
    test_gate(|a, _| a, |a: &Ciphertext, _, _, _| gates::hom_copy(a));
  }

  #[test]
  fn test_hom_constant() {
    let test = true;
    test_gate(|_, _| test, |_: _, _, _, _| gates::hom_constant(test));
  }
  ...
  fn test_hom_nor() {
    test_gate(
      |a, b| !(a | b),
      |a: &Ciphertext, b: &Ciphertext, k: &CloudKey, p: &mut FFTPlan| gates::hom_nor(a, b, k, p),
    );
  }
  ...
  fn test_hom_and_ny() {
    test_gate(
      |a, b| !a & b,
      |a: &Ciphertext, b: &Ciphertext, k: &CloudKey, p: &mut FFTPlan| gates::hom_and_ny(a, b, k, p),
    );
  }
  ...
  fn test_hom_and_yn() {
    test_gate(
      |a, b| a & !b,
      |a: &Ciphertext, b: &Ciphertext, k: &CloudKey, p: &mut FFTPlan| gates::hom_and_yn(a, b, k, p),
    );
  }
  #[test]
  fn test_hom_or_ny() {
    test_gate(
      |a, b| !a | b,
      |a: &Ciphertext, b: &Ciphertext, k: &CloudKey, p: &mut FFTPlan| gates::hom_or_ny(a, b, k, p),
    );
  }
  ...
  fn test_hom_or_yn() {
    test_gate(
      |a, b| a | !b,
      |a: &Ciphertext, b: &Ciphertext, k: &CloudKey, p: &mut FFTPlan| gates::hom_or_yn(a, b, k, p),
    );
  }

  fn test_gate<
    E: Fn(bool, bool) -> bool,
    C: Fn(&Ciphertext, &Ciphertext, &CloudKey, &mut FFTPlan) -> Ciphertext,
  >(
    expect: E,
    actual: C,
  ) {
    ...
    for _i in 0..try_num {
      ...
      let tlwe_op = actual(&tlwe_a, &tlwe_b, &cloud_key, &mut plan);
