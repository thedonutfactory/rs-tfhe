...
use rs_tfhe::lut::{Generator, LookupTable, Encoder};
...
fn main() {
    ...
    println!("Binary generator created with message modulus: {}", binary_generator.message_modulus());
    println!("Four-bit generator created with message modulus: {}", four_bit_generator.message_modulus());
    ...
    println!("Identity function LUT generated (empty: {})", identity_lut.is_empty());
    ...
    println!("Constant function LUT generated (empty: {})", constant_lut.is_empty());
    ...
    println!("Increment function LUT generated (empty: {})", increment_lut.is_empty());
    
    // Double function (mod 4)
    let double = |x: usize| (2 * x) % 4;
    let double_lut = four_bit_generator.generate_lookup_table(double);
    println!("Double function LUT generated (empty: {})", double_lut.is_empty());
    ...
    // Copy LUT2 to LUT1
    lut1.copy_from(&lut2);
    println!("After copying LUT2 to LUT1, LUT1 empty: {}", lut1.is_empty());
    ...
    println!("Full programmable bootstrapping requires integration with the blind rotation algorithm.");
